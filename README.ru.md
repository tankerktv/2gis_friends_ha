...
**Как сохранить историю.** История в Home Assistant привязана к
идентификатору сущности, а не к устройству. Поэтому:

1. запиши идентификаторы сущностей неживого устройства
   (например, `device_tracker.имя_друга`);
2. удали неживое устройство — идентификаторы освободятся;
3. открой сущности живого устройства и переименуй их в освободившиеся
   (у них сейчас, скорее всего, суффикс `_2`).

После этого история склеится: старая и новая окажутся под одним
идентификатором. Разрыв за те дни, что друг не приходил, останется — данных
за них просто нет.
