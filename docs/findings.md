...
| movement | `movement.status` | str | `stopped` (др. значения не наблюдались) |
...
- **значения `movement.status`, кроме `stopped`** — за прогон друзья не двигались,
  так что `moving`/`unreliable` предполагаются, но не наблюдались;
