...
### Свой аккаунт тоже в списке

`public_user_id` из `users/me` присутствует в `profiles[]` и `states[]` наравне
с друзьями, с `isFriend: true`. То есть за собой можно наблюдать без каких-либо
дополнительных настроек — просто ещё одно устройство в Home Assistant.

### Частота обновлений

Замер 30.07.2026, окно 5 минут, 5 друзей, **все стояли на месте**
(`stopped` / `noGeo`):

| | |
|---|---|
| фреймов `friendState` | 12 |
| уникальных изменений после дедупликации | 12 |
| отсеяно дублей | 29 % |
| изменений в минуту на всех друзей | 2,4 |
| интервал у одного друга | медиана 68 с, разброс 3–236 с |

Структура интервалов у всех одинаковая: `5, 235` / `6, 234` / `3, 236`. То есть
**всплеск сразу после подключения, затем цикл примерно раз в 4 минуты**.
Всплеск объясняется тем, что сервер отдаёт `initialState` в ответ на
`viewportChanged`, поэтому после старта данные приходят мгновенно.

Настройки периодичности не существует и существовать не может: обновления
push'ом, `update_interval=None`. Частоту целиком определяет 2ГИС.

> **Не измерено:** как часто приходят апдейты у **движущегося** друга. За все
> прогоны никто не двигался. Логично ожидать заметно чаще, но это
> предположение.

**Объём в базе:** 2,4 изменения в минуту ≈ 3500 записей в сутки по трекерам,
плюс сенсоры батареи и `last_seen`. За 10 суток хранения по умолчанию — порядка
100 тысяч строк; при `purge_keep_days: 90` уже под миллион.
